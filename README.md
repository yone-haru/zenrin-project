...
- **危険度ランク**: 危険地点の危険度（1〜5）と住民通報を距離あたりのペナルティに換算して0〜100点で採点し、「安全／注意／危険／非常に危険／要改善」の5段階ランクで表示（`backend/app/services/safety_score.py`、ランク区分はフロント `utils/grade.ts`）
- **道路構造の評価**: 歩道の有無・幹線道路・制限速度・信号や横断歩道のない交差点に加え、一時停止標識の考慮・五差路など変則交差点・急カーブ（ルート形状から検出）を加点評価
